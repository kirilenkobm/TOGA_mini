#!/usr/bin/env python3
"""Create a processed pseudogenes annotation track."""
import argparse
import sys
import os
...
DEF_SCORE = 100
PROCESSED_PSEUDOGENE_SCORE_MARK = "-2.0"
...
def get_pp_gene_chains(chain_class_file):
    ...
    f = open(chain_class_file, "r")  # open a file with classifications
    ...
    for line in f:
        ...
        # if pp_genes_field == "0":
        #     # it 0 -> no processed pseudogene chains -> skip
        #     continue
        ...
        stat = line_data[2]
        if stat == PROCESSED_PSEUDOGENE_SCORE_MARK:
...
def get_corr_q_regions(gene_to_pp_chains, chain_file, chain_dict, bed_bdb):
    ...
    2) the chain covers the processed pseudogene and nothing else.
    """
    ...
    for num, (gene, chains) in enumerate(gene_to_pp_chains.items()):
        ...
        for chain_id in chains:
            ...
            if q_strand == "-":
                # if q_strand is - we need to invert the coordinates,
...
def make_bed_lines(proj_to_reg):
    ...
    for name, region in proj_to_reg.items():
        ...
        # we don't need to score field -> so there is some default value
        line_data = (
            chrom,
            chrom_start,
            chrom_end,
...
def create_processed_pseudogenes_track(chain_class_file, chain_file, bed_bdb, output):
    """Create a processed pseudogenes track."""
    ...
    if len(gene_to_pp_chains) == 0:
        ...
        return
    # for each gene-chain pair get a corresponding region in the query
